...
Je partage ici mon expérience de doctorat que vient résumer une thèse, actuellement en fin de rédaction <strike style='color:rgb(196, 43, 94);'><span class="rayure">ou qui ne finit pas d'en finir</span></strike>, plutôt que de vous assommer avec le résumé d'une thèse qui ne représente qu'une production écrite dont je me garde la charge synthétique pour la soutenance. 
...
Au grand désespoir de mon directeur qui m'a tout de même accepté au doctorat et engagé à la Chaire pour coordonner le [projet d'édition numérique collaborative de l'Anthologie grecque](https://anthologiagraeca.org/), ma thèse ne porte pas sur l'Anthologie grecque ni sur ce projet qui est aujourd'hui coordonné par Mathilde Verstraete.
...
[^2]: Je le disais déjà [ici](https://blank.blue/cherches/je-necris-pas-ma-these-je-la-fais/) et il y avait déjà la présence massive du café dans ma vie.
...
1. la recherche ne se résume pas à des concepts, mais comprends des réalités très triviales ; 

2. l'écriture, qu'il s'agisse d'un courriel, d'une description de projet, d'un plan de cours, est déterminée par des conjonctures très concrètes, qui ne sont certainement pas toutes humaines et dont la portée nous échappent. 
...
Si l'exercice a déjà été fait par plusieurs autres thèses de l'Université de Montréal (notamment [celle de Karianne Trudeau-Desnoyers](https://papyrus.bib.umontreal.ca/xmlui/handle/1866/31925)), il n'y a pas de modèle pour une thèse recherche-création sans bipolarité : il y a déjà peu de modèle pour une thèse recherche-création "normale" (et c'est tant mieux parce que cela laisse l'espace pour penser une méthologie propre).
...
Pour autant, mes collègues de travail pourront en témoigner, je ne respecte aucun des manuels -- que je ne lis déjà pas, mes pratiques sont miennes mais ne sont pas particulièrement bonnes, je n'indente pas mon code, j'ai une centaine de modifications en attente de `commit`, j'écris mon `html` à la main (ce qui est une hérétique cause d'erreurs continuelles), je bricole et traficote et si au final *ça* marche c'est sur le fil, et on ne sait ni trop comment ni trop pourquoi. 
...
Articulé à cette traversée médiatique, j'ai développé avec l'aide d'un ami, [Enzo Poggio](https://github.com/EPgg92), un [programme pour générer des caligrammes à partir du texte](https://github.com/EPgg92/Caligram-express) même de la thèse, caligrammes dont les formes changent au fil des lignes. 
